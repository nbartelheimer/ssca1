...
void
display_elapsed(struct timeval* start_time)
{
  struct timeval now;
  ...
  printf("\n\tElapsed time: %li hour(s), %li minute(s), %li second(s), %li "
         ...
         hours_elapsed, minutes_elapsed, seconds_elapsed, miliseconds_elapsed,
         u_elapsed);
}
...
#include <mpi.h>
  ...
  if(global_parameters.ENABLE_VERIF || global_parameters.CONSTANT_RNG)
  {
    ...
    //  2613174141 -- segfault caused by insert_validation producting two
    //  identical values -550696422 -- segfault caused by the RNG producing 0.
    random_seed = (unsigned int)2613174141;
  }
  else
  {
    random_seed =
        (unsigned int)time(NULL); /* casting from time_t to unsigned int we can
                                     lose precision... no big deal here */
  ...
  if(rank == 0)
  {
    printf("HPCS SSCA #1 Bioinformatics Sequence Alignment Executable "
           "Specification:\nRunning...\n");
  ...
  */
  ...
  sim_matrix = gen_sim_matrix(
      global_parameters.SIM_EXACT, global_parameters.SIM_SIMILAR,
      global_parameters.SIM_DISSIMILAR, global_parameters.GAP_START,
      global_parameters.GAP_EXTEND, global_parameters.MATCH_LIMIT);

  seq_data = gen_scal_data(sim_matrix, global_parameters.MAIN_SEQ_LENGTH,
...
#if defined(USE_MPI) || defined(USE_GASPI)
  QUIET();
#endif
  A = pairwise_align(seq_data, sim_matrix, global_parameters.K1_MIN_SCORE,
  ...
  if(rank == 0)
  {
    ...
    scanBackward(A, global_parameters.K2_MAX_REPORTS,
