...
int
verify_alignment(good_match_t* A, int maxDisplay)
{
  int score;
  int retval = 0;
  ...
  if(A->bestLength == 0)
  {
    ...
    return retval; // returning sucess here because the alignment didn't
                   // actually fail...
  }
  else
  {
    printf("\nFound %lu acceptable alignments with scores from %i to %i.\n",
           A->bestLength, (int)A->bestScores[0],
  ...
  for(int m = 0; m < A->bestLength; m++)
  {
    ...
    {
      retval = 1;
      printf("\nverifyAlignment %i failed; Kernel 1 reported %i versus Kernel "
             "2 reported %i:\n---------------------------\n",
    ...
    if(m < maxDisplay)
    {
      ...
      assemble_acid_chain(A, main_acid_chain, A->bestSeqs[m].main,
                          A->bestSeqs[m].main->length);
      assemble_acid_chain(A, match_acid_chain, A->bestSeqs[m].match,
                          A->bestSeqs[m].match->length);

      assemble_codon_chain(A, main_codon_chain, A->bestSeqs[m].main,
                           A->bestSeqs[m].main->length);
      assemble_codon_chain(A, match_codon_chain, A->bestSeqs[m].match,
                           A->bestSeqs[m].match->length);

      printf("%7ld  %s  %s  %7ld\n%7ld  %s  %s  %7ld\n", A->bestStarts[0][m],
             main_acid_chain, main_codon_chain, A->bestEnds[0][m],
             A->bestStarts[1][m], match_acid_chain, match_codon_chain,
             A->bestEnds[1][m]);
    }
  }
  return retval;
 ...
 *   depth          [int] used to decide how big the outputs will be and for
 * debugging OUTPUT rs             [i, j] coordinates of end of path or -1 if
 * there is no path ri             [int[M]] main sequence rj             [uint8
 * row vector] match sequence
 */

/* Matlab allows for a = [1 2] and b = [0 a] and b will be [0 1 2].  Obviously
 * we can't have that in C. */
void
tracepath(good_match_t* A, int sizeT, int* T, int ei, int ej, int i, int j,
          int dir, int rs[2], seq_data_t* sequence, int depth)
{
  codon_t Ci, Cj;
  ...
  */
  ...
  if(dir == 0 || (dir & T[index2d(i, j, sizeT)])) // if we aren't skipping and
                                                  // it isn't part of a gap
...
void
doScan(good_match_t* A, int* bestR, int minSeparation, int report)
{
  ...
  // initialize the V, E, and F arrays.  We use the smallest possible value that
  // won't be underflowed by the algorithm.
  ...
  while(fi > 0) // loop over diagnal starting positions
  {
    ...
    while(dj <= lj && di > 0 && e < sizeT && f < sizeT)
    {
      ...
      if(s == goal)
      {
        ...
        if(rs[0] == -1 &&
           rs[1] == -1) // tracepath returns -1 in rs if there is no path
        {
  ...
  // see the note in the function header regarding sequences that could not be
  // found
...
/*
 * This function really just sets up memory and invokes the main loop until we
 * have the required number of reports. The real meat is in doScan.
 */
void
scanBackward(good_match_t* A, int maxReports, int minSeparation)
{
