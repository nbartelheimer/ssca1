...
static index_t
unsigned_abs_diff(index_t A, index_t B)
{
  return (A > B) ? (A - B) : (B - A);
}

void
print_back_20(good_match_t* A, seq_t* in_seq, index_t end_index)
{
  ...
  printf("%7ld  %s  %s  %7ld\n", start, main_acid_chain, main_codon_chain,
         end_index);
}

static void
considerAdding(score_t score, int minSeparation, index_t main_index,
               index_t match_index, int maxReports, current_ends_t* score_ends)
{
  ...
  if(score_ends->report == score_ends->size)
  {
    ...
    if(sorted_array == NULL)
      sorted_array = (score_t*)malloc(score_ends->size * sizeof(score_t));
    if(best_index == NULL)
      best_index = (index_t*)malloc(score_ends->size * sizeof(index_t));

    memcpy(sorted_array, score_ends->goodScores,
    ...
    for(int index_for_index = worst_keeper; index_for_index < score_ends->size;
        index_for_index++)
    {
...
void
release_good_match(good_match_t* doomed)
{
  if(doomed == NULL)
    return;
...
static score_matrix_t*
alloc_score_matrix(index_t matrix_length)
{
...
static gap_matrix_t*
alloc_gap_matrix(index_t matrix_length)
{
...
static void
free_score_matrix(score_matrix_t* doomed)
{
...
static void
free_gap_matrix(gap_matrix_t* doomed)
{
  FREE_ALL(doomed->scores);
  free(doomed);
}
...
static void
fetch_score(score_matrix_t* A, index_t m, index_t n, score_t* in)
{
  int target_ep = n / A->local_length;
  int local_index = n % A->local_length;

  SHORT_GET(in, &(A->scores[index2d(m % 3, local_index, A->local_length)]), 1,
            target_ep);
}

static void
fetch_gap(gap_matrix_t* A, index_t m, index_t n, score_t* in)
{
  int target_ep = n / A->local_length;
  int local_index = n % A->local_length;

  SHORT_GET(in, &(A->scores[index2d(m % 2, local_index, A->local_length)]), 1,
            target_ep);
}

/* maybe useful for going to further extremes. Commented out because GCC
 * complains of unused functions */
#if 0
...
static void
assign_score(score_matrix_t* A, index_t m, index_t n, score_t new_value)
{
  int target_ep = n / A->local_length;
  int local_index = n % A->local_length;

  SHORT_PUT(&(A->scores[index2d(m % 3, local_index, A->local_length)]),
            &new_value, 1, target_ep);
}

static void
assign_gap(score_matrix_t* A, index_t m, index_t n, score_t new_value)
{
  int target_ep = n / A->local_length;
  int local_index = n % A->local_length;
...
static void
collect_best_results(current_ends_t** good_ends, int max_reports,
                     int max_threads, good_match_t* answer)
{
  index_t copied = 0;
  int max_values = 0;
  ...
  for(int idx = 0; idx < num_nodes; idx++)
  {
    ...
    SHORT_GET(&(collected_ends.goodScores[copied]), good_ends[0]->goodScores,
              current_end.report, idx);
    LONG_GET(&(collected_ends.goodEnds[0][copied]), good_ends[0]->goodEnds[0],
             current_end.report, idx);
    LONG_GET(&(collected_ends.goodEnds[1][copied]), good_ends[0]->goodEnds[1],
             current_end.report, idx);
...
/* pairwise_align
 * real meat of the program, this function finds codon similarities in seq_data
 *using the matrix sim_matrix Input: seq_data_t *seq_data     - Sequence data
 *generated by genScalData() sim_matrix_t *sim_matrix - Codon similarity matrix
 *generated by genSimMatrix() int minScore             - Minimum end point
 *score, from the init_parameters() function int maxReports           - Maximum
 *number of reports to keep, from the init_parameters() function int
 *minSeparation        - Minimum end point seperation in codons, from the
...
good_match_t*
pairwise_align(seq_data_t* seq_data, sim_matrix_t* sim_matrix,
               const int minScore, const int maxReports,
  ...
  for(index_t idx = 2; idx < seq_data->match->length * 2 - 1; idx++)
  {
    ...
    if(idx < seq_data->match->length)
    {
      if(rank == 0)
      {
        ...
        if((new_score > good_ends[omp_get_thread_num()]->min_score && W > 0 &&
            new_score == G))
        {
          if(m + 1 == seq_data->main->length || n == 0)
          {
            considerAdding(new_score, minSeparation, m, n, maxReports,
    ...
    // As a note, this loop is the program execution time. If you're looking to
    // optimize this benchmark, this is all that counts.
