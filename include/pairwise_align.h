...
typedef struct _seq_t
{
  ...
  codon_t *match;
  index_t length;
  index_t backing_memory; //NOTE: right now this is only used in multipleAlign,
before that backing memory is the same as length } seq_t;
*/
...
good_match_t* pairwise_align(seq_data_t* seq_data, sim_matrix_t* sim_matrix,
                             int K1_MIN_SCORE, int K1_MAX_REPORTS,
