...
void
distribute_rng_seed(unsigned int new_seed)
{
#ifdef USE_SHMEM
...
void
seed_rng(int adjustment)
{
  srand(random_seed + adjustment);
}

void
extend_seq(seq_t* extended, index_t extend_size)
{
...
void
touch_memory(void* mem, size_t size)
{
...
/* Fix up a sequence to remove the gaps
   Input-
        good_match_t *A    - the match struct for the sequence.  Used for the
   hyphen member int *dest          - destination array.  Needs to be allocated
   before it is passed in int *source        - sequence to be scrubbed int
   length         - length of the source.  Dest should be the same size Output-
        int *dest          - will be full of a gapless sequence
        int (return value) - size of dest
*/

index_t
scrub_hyphens(good_match_t* A, seq_t* dest, seq_t* source, index_t length)
{
...
/* Helper function for displaying acid chains.  This function will take a chain
   and make an ascii representation of the acids as defined in A Input-
...
void
assemble_acid_chain(good_match_t* A, char* result, seq_t* chain, index_t length)
{
...
/* Helper function for displaying codon chains.  This function will take a chain
   and make an ascii representation of the codon sequence out of it Input-
       good_match_t *A    - the match struct for the sequence.
...
void
assemble_codon_chain(good_match_t* A, char* result, seq_t* chain,
                     index_t length)
{
  memset(result, '\0', length);
  if(length > chain->length)
...
score_t
simple_score(good_match_t* A, seq_t* main, seq_t* match)
{
  score_t score = 0;
  int mainMatch = 1;
  int matchMatch = 1;
  ...
  for(int i = 0; i < length; i++)
  {
    ...
    matchMatch = 1;
    score =
        score + A->simMatrix->similarity[main->sequence[i]][match->sequence[i]];
  }
  return score;
}
