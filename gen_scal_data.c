...
char validations[2][3][32] = {
    {"ACDEFG*IDENTICAL*HIKLMN", "ACDEFG*MISQRMATCHES*HIKLMN",
     "ACDEFG*STARTGAPMIDSTEND*HIKLMN"},
    {"MNLKIH*IDENTICAL*GFEDCA", "MNLKIH*MISRQMATCHES*GFEDCA",
     "MNLKIH*STARTMIDSTGAPEND*GFEDCA"}};

/* print the output */

void
verifyData(sim_matrix_t* simMatrix, seq_data_t* seqData)
{
  printf("\n");
...
index_t*
gen_indexes(int num_indexes, index_t rand_max, index_t min_seperation)
{
  int not_done;
...
void
create_sequence(seq_t* sequence, char validations[][32], int num_validations,
                sim_matrix_t* simMatrix)
{
  ...
  if(rank == 0)
  {
    for(int idx = 0; idx < num_validations; idx++)
    {
      ...
      {
        write_to_seq(sequence, indexes[idx] + jdx,
...
seq_data_t*
gen_scal_data(sim_matrix_t* simMatrix, index_t mainLen, index_t matchLen,
              int constant_rng)
{
...
void
release_scal_data(seq_data_t* doomed_seq)
{
