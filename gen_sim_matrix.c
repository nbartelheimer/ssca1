...
static const char* l_similarity[] = {"L",   "ttg", "tta", "ctt",
                                     "ctc", "cta", "ctg"};
...
static const char* r_similarity[] = {"R",   "cgt", "cgc", "cga",
                                     "cgg", "aga", "agg"};
static const char* s_similarity[] = {"S",   "tct", "tcc", "tca",
                                     "tcg", "agt", "agc"};
...
sim_matrix_t*
gen_sim_matrix(int exact, int similar, int dissimilar, int gapStart,
               int gapExtend, int matchLimit)
{
  char process_acid;
  int ccode = 0;
  ...
  // strcpy(the_similarity_matrix->codon[64],
  // {(char)HYPHEN,(char)HYPHEN,(char)HYPHEN});
      ...
      similarity_matrix_index++)
  {
    ...
    for(int codon_index = 1;
        codon_index < similarity_length[similarity_matrix_index]; codon_index++)
    {
      ccode = 0;
      for(int base_index = 0; base_index < 3; base_index++)
      {
        ...
        default:
          fprintf(stderr,
                  "Wandered into territory I wasn't supposed to, aborting.\n");
          abort();
          break;
        }
      }
  ...
  /* We don't get fancy vector notation like in matlab, so let's do all the work
   * in one big loop */
...
void
release_sim_matrix(sim_matrix_t* doomed_matrix)
{
  free(doomed_matrix);
}
